...
function getAdPlanCycleList(){
		...
		data : {adPlanId : $('#adPlanId1').val(),qryuserGroup : $('#qryuserGroup1').val()},
		dataType : "json",
		cache : false,
		success : function(temp){
			$('#select2 option').remove(); 
			$("#select2").append($("<option>").text("全部").val("") );
			for(var i=0;i<temp.length;i++){ 
			_option = $("<option>").text(temp[i].adPlanCycleName).val(temp[i].adPlanCycleId);
			$("#select2").append(_option); 
			}
			if($("#adPlanCycleId1").val() !=""){
				$("#select2").attr('value' ,$("#adPlanCycleId1").val()); 	
			$("#adPlanCycleId1").val("");
			}
		}
	});
...
<!-- 分页 -->
<form id="pagerForm" method="post" action="${pageContext.request.contextPath }/dana.do?action=pieDetailAction&flag=pager" >
...
<div class="page">
	<div class="pageHeader">
		<form onsubmit="return navTabSearch(this);" action="${pageContext.request.contextPath }/dana.do?action=pieDetailAction" method="post">
		...
		<div class="searchBar">
			<ul >		
				<li> 
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					计划：
					<select id="adPlanId1"  name="adPlanId" style="width: 120px;" onchange="getAdPlanCycleList()">
						<option value="">全部</option>
					...
					&nbsp;&nbsp;&nbsp;&nbsp;
					周期：
					<select id="select2" name="adPlanCycleId" style="width: 120px;">
						<option value="">全部</option>
	...
	<form method="post" action="${pageContext.request.contextPath }/dana.do?action=pieDetailAction" class="pageForm required-validate" onsubmit="return validateCallback(this);">
		<table class="table" layouth="400">
			...
			<tbody>
				...
				<tr> 
					<td><c:if test="${adboxc[5]==null }">其他</c:if>
					<c:if test="${adboxc[5]!=null }">${adboxc[5]}</c:if>
					</td>
					<td>${adboxc[0]}</td>
					<td>${adboxc[1]}</td>
					<td>${adboxc[2]}</td>
		...
		<table align="center" width="95%">
			<tr >
				<td>
					<div id="chartdivp" align="center">
						FusionCharts.
					</div>
					<script type="text/javascript">
						var chart = new FusionCharts("${pageContext.request.contextPath}/charts/FCF_Pie2D.swf", "ChartId", "400", "300");
						chart.setDataXML("${dataXMLp}");	   
						chart.render("chartdivp");
					</script>
				</td>
			</tr>
		</table>
