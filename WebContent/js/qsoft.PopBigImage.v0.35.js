/**
    ...
    **/
    ...
    qsoft.PopBigImage = function (origImage,dx,dy,mx,my)
    {
        ...
        var mainWidth = document.body.offsetWidth/2;
        var mainHeight = document.body.offsetHeight/2;
//        alert("屏幕可用工作区域宽度："+window.screen.availWidth);
//        alert("网页正文宽度："+document.body.scrollWidth);
//        alert(mainWidth);
        ...
        if(rect.left <= mainWidth){
        this.ol = rect.left + this.detaX + this.ow - (qsoft.isIE ?2:0);
        }else{
        this.ol = rect.left - this.detaX -  this.maskX - (qsoft.isIE ?2:0);
        }
        if(rect.top<=mainHeight){
        this.ot = rect.top + this.detaY - (qsoft.isIE ?2:0);
        }else{
        this.ot = rect.top + this.detaY - this.oh - (qsoft.isIE ?2:0);
        }
        var qObj = this;
        this.createMask = function ()
        {
            if(typeof(this.mask) == "undefined")
            {
                ...
                this.mask.style.border = "5px solid silver";
